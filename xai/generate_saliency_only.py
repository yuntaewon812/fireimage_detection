...
def save_fold_visualizations(image_path, saliencies, save_dir, model_name, subclass):
    ...
    avg_sal = torch.mean(torch.stack(saliencies), dim=0).squeeze().cpu().numpy()
    avg_sal = (avg_sal - avg_sal.min()) / (avg_sal.max() - avg_sal.min() + 1e-8)
    plt.figure(figsize=(6, 6))
    plt.imshow(img_np)
    plt.imshow(avg_sal, cmap='hot', alpha=0.6)
...
def run_all_classes(model_name, num_samples_per_subclass=10):
    for target_class in class_names:
        ...
        print(f"  Running Saliency on {len(selected_paths)} images...")
            ...
            try:
                ...
                for fold_num in [0, 1, 2]:
                    ...
                    if not os.path.exists(model_path):
                        ...
                        continue
                    model = build_model(model_name, model_path)
                    saliencies.append(compute_saliency_map(model, input_tensor))
                    del model
                    torch.cuda.empty_cache()

                if saliencies:
                    save_fold_visualizations(img_path, saliencies,
                                             base_result_path, model_name, subclass)
...
if __name__ == "__main__":
    ...
    for model_name in TOP2_MODELS:
        print(f"\n{'='*50}")
        print(f"Saliency: {model_name}  |  device: {device}")
        print('='*50)
        run_all_classes(model_name, num_samples_per_subclass)
    print("\nAll Saliency processing complete!")
