...
class NCB(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, path_dropout=0,
        ...
        self.attention_path_dropout = nn.Dropout(path_dropout)

        self.norm = norm_layer(out_channels)
        self.mlp = Mlp(out_channels, mlp_ratio=mlp_ratio, drop=drop, bias=True)
        self.mlp_path_dropout = nn.Dropout(path_dropout)
...
class NTB(nn.Module):
    ...
    def __init__(
        ...
        self.mhsa_path_dropout = nn.Dropout(path_dropout * mix_block_ratio)

        self.projection = PatchEmbed(self.mhsa_out_channels, self.mhca_out_channels, stride=1)
        self.mhca = MHCA(self.mhca_out_channels, head_dim=head_dim)
        self.mhca_path_dropout = nn.Dropout(path_dropout * (1 - mix_block_ratio))
...
class NextViTForImageClassification_v2(nn.Module):
    ...
    def __init__(self, num_labels=7, img_size=224, patch_size=16, hidden_dim=512,
                 model_variant='small'):
        super(NextViTForImageClassification_v2, self).__init__()
    ...
    def forward(self, images, labels=None):
        ...
        features = self.safe_module(features)
        # Classification
