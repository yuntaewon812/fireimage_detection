...
class SwinTransformerStage(nn.Module):
    ...
    def __init__(self,
        ...
        # Get block - simplified to use SwinTransformerBlock only
...
class SwinTransformerV2ForImageClassification_v2(nn.Module):
    """SwinTransformerV2 wrapper for image classification compatible with the project"""

    def __init__(self, num_labels=7, img_size=224, patch_size=16, hidden_dim=512,
                 model_variant='tiny'):
        super(SwinTransformerV2ForImageClassification_v2, self).__init__()
        ...
        config = configs.get(model_variant, configs['tiny'])
        ...
        self.backbone = SwinTransformerV2(
            ...
            patch_size=4,  # Standard patch size for SwinV2
            ff_feature_ratio=4,
            dropout=0.0,
        ...
        # Calculate output dimension - final stage has embedding_channels * 2^3
        num_features = config['embedding_channels'] * (2 ** (len(config['depths']) - 1))
    ...
    def forward(self, images, labels=None):
        # Extract features - get the final stage output
        stage_features = self.backbone(images)
        final_features = stage_features[-1]  # Last stage features

        # Global average pooling
        features = self.avgpool(final_features)
...
class SwinTransformerV2ForImageClassification(nn.Module):
    """SwinTransformerV2 wrapper for image classification compatible with the project"""
    
    def __init__(self, num_labels=7, img_size=224, patch_size=16, hidden_dim=512, 
                 model_variant='tiny'):
        super(SwinTransformerV2ForImageClassification, self).__init__()
